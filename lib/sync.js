...
class Syncer {
  ...
  sync () {
    if (this.syncInProgress) {
      ...
      this.getHead(this.srcDir).then(({ref, sha}) => {
        const message = `git-n-sync commit, you probably shouldn't be seeing this\n\n${ref} ${sha}`
        this.commitWorkingDir(sha, message).then((commitHash) => {
          utils.cmd(this.srcDir, `git update-ref refs/${SYNCER_REF} ${commitHash}`).then(() => {
            const command = `git push --force #{@remote} refs/${SYNCER_REF}:refs/${SYNCER_REF}`
            utils.cmd(this.srcDir, command).then(({stdout, stderr}) => {
              if (this.verbose) {
                console.log(stdout)
                console.error(stderr)
              }

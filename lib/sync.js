...
class Syncer {
    ...
    return this.getHead(this.srcDir).then(({ref, sha}) => {
      ...
      return this.commitWorkingDir(sha, message).then((commitHash) => {
        return utils.cmd(this.srcDir, `git update-ref refs/${SYNCER_REF} ${commitHash}`).then(() => {
          ...
          return utils.cmd(this.srcDir, command, {env: {GIT_SSH: path.join(__dirname, '..', 'ssh.sh')}}).then(({stdout, stderr}) => {
            if (this.verbose) {
              console.log(stdout)
              console.error(stderr)
            }
            ...
            tray.setDoneSyncing(this.srcDir)
            if (this.nextSync) {
  ...
  processRemoteOutput (remoteOutput) {
    ...
    const summaryDiff = summaryDiffRegex.exec(output)[1].split('\n')
    let updates = []
    for (const index in summaryDiff) {
      const line = summaryDiff[index]
